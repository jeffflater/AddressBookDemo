...
namespace AddressBook.Lib.BLL
{
    public class RelationshipBll : IRelationshipBll
    {
        ...
        public IEnumerable<Leaf> GetAll(long id, PersonType personType)
        {
            ...
            var trees = AdoProvider.QueryTransaction<Tree>(sql.ToString());
        ...
        private static void TraverseTree(IEnumerable<Tree> trees)
        {
            while (true)
            {
                ...
                foreach (var tree in trees)
                {
                    ...
                    var treeSearcher = AdoProvider.QueryTransaction<Tree>(sql.ToString());
        ...
        public void Save(Tree tree)
        {
            ...
            sql.Append(
                string.Format("SELECT ChildBranch FROM dbo.RelationshipTree WHERE ParentId = {0} AND ParentPersonType = {1}",
                    parentId, parentPersonTypeId));
